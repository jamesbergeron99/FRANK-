...
<body onclick="initAudio()">
    ...
    <script>
        ...
        // GREETING RECTIFIED
        const welcomeText = "Hey there, I'm Frank. Not just in name, but in spirit, I’m your AI script doctor. I’m here to give you actionable feedback on your features or TV series, including a full spelling and grammar check, logline, synopsis, and my 18-point forensic audit. Or, pick TV series for additional coverage over multiple episodes. Darling, it takes time to process brilliance, so chill and give me a minute.";
            ...
            try {
                ...
                parts.forEach(async (part, index) => {
                    const ttsResp = await fetch("https://api.inworld.ai/tts/v1/voice", {
                        ...
                        body: JSON.stringify({ text: part, voiceId: "default-oglabcjnetcklcq7rghmbw__frank", modelId: "inworld-tts-1.5-max" })
                    });
        ...
        async function uploadFiles() {
            ...
            addMessage(`Analyzing ${mode}...`, false);
        ...
        async function sendBanter() {
            ...
            textInput.value = ''; addMessage(val, true);
        ...
        async function speak(fullText) {
            ...
            sections.forEach(async (section, index) => {
                ...
                try {
                    const ttsResp = await fetch("https://api.inworld.ai/tts/v1/voice", {
                        method: "POST", headers: { "Authorization": `Basic ${settings.apiKey}`, "Content-Type": "application/json" },
                        body: JSON.stringify({ text: section, voiceId: "default-oglabcjnetcklcq7rghmbw__frank", modelId: "inworld-tts-1.5-max" })
                    });
                    const data = await ttsResp.json();
                    audioBuffers[index] = await audioCtx.decodeAudioData(Uint8Array.from(atob(data.audioContent), c => c.charCodeAt(0)).buffer);
        ...
        function playSequentially() {
            ...
            if (audioBuffers[nextToPlay]) {
                ...
                currentSource.onended = () => { if (isPlaying && isSequenceActive) { nextToPlay++; playSequentially(); } };
                currentSource.start();
        ...
        function togglePlayback() { 
            if (isPlaying) { stopCurrentAudio(); document.getElementById('playPauseBtn').innerHTML = '<i class="fas fa-play"></i>'; }
            else { isSequenceActive = true; playSequentially(); document.getElementById('playPauseBtn').innerHTML = '<i class="fas fa-pause"></i>'; } 
        }
