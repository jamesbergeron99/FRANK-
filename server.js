...
3. 18-POINT NARRATIVE AUDIT: Numbered 1-18. Each point must be LABELED and responded to with a full, insightful paragraph weaving in page-specific quotes. Cite at least TWO specific [Page X] locations for every point.
...
app.post('/analyze', upload.array('scripts', 10), async (req, res) => {
    try {
        ...
        const scanResults = await Promise.all(chunks.map(chunk => 
            model.generateContent(`Extract 15 significant dialogue quotes, specific typos, and formatting errors: \n\n ${chunk}`)
        ));
