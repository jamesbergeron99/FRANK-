...
// HEALTH CHECK (To prevent Render timeout)
app.get('/health', (req, res) => res.status(200).send('Frank is ready.'));
...
app.post('/analyze', upload.array('scripts', 10), async (req, res) => {
    ...
    if (!req.files || req.files.length === 0) {
        return res.status(400).json({ message: "Where are the pages, darling?" });
    }
    ...
    try {
        ...
        res.status(500).json({ message: "Frank is indisposed, honey. Check the logs." });
    }
});
